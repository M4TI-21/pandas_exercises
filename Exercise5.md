...
```python
### Your code here
p = plt.hist(df_pokemon['Defense'], bins=10)
```
...
```python
### Your code here
types = np.unique(df_pokemon['Type 1'])
p = plt.hist(df_pokemon['Type 1'], bins=len(types))
plt.show()
```
...
```
    ...
    <matplotlib.patches.FancyArrow at 0x24c8fb61550>
...
#normal distribution - the biggest amount of data is in the middle of the graph
#skewed distribution - opposite of normal, the largest amount is on the sides of graph

#outliers - data pieces that are not connected to the rest (i.e. errors or individuals)
```
...
```
    ...
    <matplotlib.lines.Line2D at 0x24c92a2d450>
...
#the Y values are decreasing as X are increasing, because of the -x * 2 formula.
#it is called the negative linear correlation
```
...
```python
### Check-in
plt.scatter(df_pokemon['Speed'], df_pokemon['Attack'], alpha=0.5)

# The best option is to use scatterplot, as it clearly illustrates non-linear differences.
```




    <matplotlib.collections.PathCollection at 0x24c920c6710>
...
#this plot does not inform how many pokemons are in each group
#moreover it doesn't inform why the legendary status has higher attack stats
```
