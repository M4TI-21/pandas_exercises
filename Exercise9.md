...
```python
correlation, p_value = ss.pearsonr(df_pokemon['Attack'], df_pokemon['Defense'])

print(f"Pearson correlation: {correlation}")
print(f"P-value: {p_value}")

# the relationship is moderately positive, not too strong and not too weak.
```

    Pearson correlation: 0.4386870551184896
...
df_pokemon.head()
corr_coef, p_value = pearsonr(df_pokemon["Defense"], df_pokemon["Sp. Def"])

print(f"Pearson Correlation Coefficient: {corr_coef:.4f}")
print(f"P-value: {p_value:.4e}")

plt.figure(figsize=(8, 6))
sns.regplot(x='Defense', y='Sp. Def', data=df_pokemon)
plt.title(f'Pearson Correlation between Defense and Sp. Def\nr = {corr_coef:.2f}, p = {p_value:.2e}')
plt.xlabel('Defense')
...
```
    ...
    Pearson Correlation Coefficient: 0.5107
    P-value: 2.3043e-54
...
# Boxplot for Quantitative and Qualitative
# Bubble plot for Quantitative and Ranked
# Bar plot for Qualitative and Ranked - sum of Pokemon stats by some category

df_pokemon = pd.read_csv('pokemon.csv')

types = df_pokemon['Type 1'].unique()
attack_data = [df_pokemon[df_pokemon['Type 1'] == type]['Attack'].values for type in types]
plt.figure(figsize=(12, 6))
plt.boxplot(attack_data, labels=types)
...
df_sales_encoded = pd.get_dummies(df_sales, columns=['Store_Type', 'City_Type', 'Product_Quality'], drop_first=True)
sns.heatmap(df_sales_encoded.corr(), annot=True)
plt.title("Correlation of numeric data and encoded categorical columns")
plt.show()
...
![png](Exercise9_files/Exercise9_56_0.png)
    



```python
sns.barplot(data=df_sales, x='Day_Temp', y='Sales')
...
![png](Exercise9_files/Exercise9_57_0.png)
